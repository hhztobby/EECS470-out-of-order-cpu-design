...
MODULES = cpu mult rob rs dispatcher integrated map_table isReg load_station sq fu store_unit cache_arbit icache fetcher dcache
...
MULT_FILES = verilog/sys_defs.svh verilog/psel_gen.sv
build/mult.simv: $(MULT_FILES)
build/mult.cov: $(MULT_FILES)
synth/mult.vg: $(MULT_FILES)

STORE_UNIT_FILES = verilog/sys_defs.svh
build/mult.simv: $(STORE_UNIT_FILES)
build/mult.cov: $(STORE_UNIT_FILES)
synth/mult.vg: $(STORE_UNIT_FILES)

# TODO: add any files required for the RS here (besides test/rs_test.sv and verilog/rs.sv)
RS_FILES = verilog/sys_defs.svh verilog/psel_gen.sv verilog/rs_defs.svh
...
DISPATCHER_FILES = verilog/sys_defs.svh verilog/psel_gen.sv verilog/decoder.sv verilog/ISA.svh
build/dispatcher.simv: $(DISPATCHER_FILES)
build/dispatcher.cov: $(DISPATCHER_FILES)
synth/dispatcher.vg: $(DISPATCHER_FILES)
...
INTEGRATED_FILES = verilog/sys_defs.svh verilog/ISA.svh\
  verilog/rs_defs.svh verilog/dispatcher_defs.svh\
  verilog/psel_gen.sv verilog/decoder.sv\
  verilog/regfile.sv verilog/regMem.sv\
  verilog/rs.sv verilog/dispatcher.sv verilog/integrated.sv\
  verilog/map_table.sv verilog/freelist.sv \
  verilog/isReg.sv verilog/fu.sv\
  verilog/alu.sv verilog/branch.sv verilog/mult.sv\
  verilog/rob.sv 
  
build/integrated.simv: $(INTEGRATED_FILES)
build/integrated.cov: $(INTEGRATED_FILES)
synth/integrated.vg: $(INTEGRATED_FILES)
# TODO: add any files required for the ALU here (besides test/rob_test.sv and verilog/rob.sv)
ALU_FILES = verilog/sys_defs.svh
build/alu.simv: $(ALU_FILES)
build/alu.cov: $(ALU_FILES)
synth/alu.vg: $(ALU_FILES)

# TODO: add any files required for the ALU here (besides test/rob_test.sv and verilog/rob.sv)
BRANCH_FILES = verilog/sys_defs.svh
build/branch.simv: $(BRANCH_FILES)
build/branch.cov: $(BRANCH_FILES)
synth/branch.vg: $(BRANCH_FILES)

ISREG_FILES = verilog/sys_defs.svh
build/isReg.simv: 	$(ISREG_FILES)
build/isReg.cov: 	$(ISREG_FILES)
synth/isReg.vg: 	$(ISREG_FILES)

# TODO: add any files required for the ALU here (besides test/rob_test.sv and verilog/rob.sv)
FU_FILES = verilog/sys_defs.svh verilog/alu.sv verilog/branch.sv verilog/mult.sv verilog/psel_gen.sv verilog/store_unit.sv verilog/load_station.sv
build/fu.simv: $(FU_FILES)
build/fu.cov: $(FU_FILES)
synth/fu.vg: $(FU_FILES)

ICACHE_FILES = verilog/sys_defs.svh verilog/memDP.sv
build/icache.simv: $(ICACHE_FILES) test/mem.sv 
build/icache.cov: $(ICACHE_FILES) test/mem.sv
synth/icache.vg: $(ICACHE_FILES)
build/icache.syn.simv: $(CPU_TESTBENCH) test/mem.sv

DCACHE_FILES = verilog/sys_defs.svh verilog/memDP.sv
build/icache.simv: $(DCACHE_FILES) test/mem.sv 
build/icache.cov: $(DCACHE_FILES) test/mem.sv
synth/icache.vg: $(DCACHE_FILES)
build/icache.syn.simv: $(CPU_TESTBENCH) test/mem.sv


LOAD_STATION_FILES = verilog/sys_defs.svh# verilog/psel_gen.sv
build/load_station.simv: $(LOAD_STATION_FILES)
build/load_station.cov: $(LOAD_STATION_FILES)
synth/load_station.vg: $(LOAD_STATION_FILES)
...
CACHE_ARBIT_FILES = verilog/sys_defs.svh
build/cache_arbit.simv: $(CACHE_ARBIT_FILES)
build/cache_arbit.cov: 	$(CACHE_ARBIT_FILES)
synth/cache_arbit.vg: 	$(CACHE_ARBIT_FILES)
...
CPU_SOURCES = verilog/cpu.sv \
	...
	verilog/ras.sv \
	verilog/sq.sv \
	verilog/store_unit.sv \
	verilog/load_station.sv \
	verilog/cache_arbit.sv \
	verilog/dcache.sv \
	verilog/cache_controller.sv
